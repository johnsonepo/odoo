...
class FeedController(http.Controller):
    ...
    @http.route('/product_data/facebook-feed.xml', type='http', auth='public')
    def serve_xml_file(self):
        products = request.env['product.template'].search([('allow_facebook_ads', '=', True)])
        ...
        for product in products:
            ...
            if product.google_category_id:
                if isinstance(product.google_category_id, str):
                    ...
                    google_category_name = html.escape(product.google_category_id.name)
            else:
            ...
            for product_variant in product.product_variant_ids:
                ...
                product_name = html.escape(product_variant.name)
                product_description = html.escape(product_variant.description or '')
                ...
                attribute_string = ','.join([f"{html.escape(name)}:{html.escape(value)}" for name, value in variant_attributes.items()])
